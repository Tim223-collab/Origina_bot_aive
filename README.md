...
│   └── work_parser_service.py # Парсинг сайта
...
Откройте `services/work_parser_service.py` и настройте селекторы в методах:
- `_login()` - селекторы формы входа
- `parse_reports()` - селекторы элементов статистики
